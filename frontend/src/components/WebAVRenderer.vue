 ...
 */
watch(
  () => unifiedStore.playbackModule.isPlaying,
  (isPlaying) => {
...
async function waitForProjectSettingsReady(): Promise<void> {
  ...
  // TODO: 等待 projectModule 集成到 UnifiedStore 后恢复项目设置检查
  // 现在直接返回，假设设置总是就绪的
  console.log('🔄 [LIFECYCLE] WebAVRenderer 设置检查跳过（使用 UnifiedStore）')
...
onMounted(async () => {
  ...
  try {
    ...
    // TODO: 等待 projectModule 集成到 UnifiedStore 后恢复项目设置检查
    console.log('🔄 [LIFECYCLE] WebAVRenderer 项目设置检查跳过（使用 UnifiedStore）')

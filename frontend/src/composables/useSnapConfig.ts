...
export function useSnapConfig() {
  ...
  // 计算属性：是否启用关键帧吸附
  const isKeyframesEnabled = computed(() => config.value.enabled && config.value.keyframes)

  // 计算属性：是否启用播放头吸附
  const isPlayheadEnabled = computed(() => config.value.enabled && config.value.playhead)

  // 计算属性：是否启用时间轴起始位置吸附
  const isTimelineStartEnabled = computed(() => config.value.enabled && config.value.timelineStart)
  ...
  const isActuallyEnabled = computed(() => config.value.enabled && !temporaryDisabled.value)

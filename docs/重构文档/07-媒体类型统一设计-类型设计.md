...
UnifiedMediaItem采用**状态容器**和**信息查询接口**的设计理念，大幅精简操作方法：

- **自动化处理**：创建后自动开始处理，无需手动调用 `startProcessing()`
- **状态机保护**：移除 `forceTransition()` 等绕过状态机的危险方法
- **管理器驱动**：暂停/恢复等复杂控制由管理器负责，不暴露给用户
- **只读为主**：大部分方法是状态查询，而非状态修改
...
所有状态变更和数据设置都必须通过 `transitionTo` 方法进行，确保：
...
以下方法已从设计中移除，改为通过转换上下文统一处理：

- ~~`setUserDuration(duration, description)`~~ → 使用 `UserInputContext`
- ~~`setServerMetadata(metadata)`~~ → 使用 `ServerMetadataContext`
...
媒体项目的duration字段采用**多来源、渐进式精确**的设计，可以在不同阶段从不同来源获得：
...
mediaItem.transitionTo(mediaItem.mediaStatus, {
  ...
  source: 'user',
  reason: 'User provided duration',
  userDuration: 1800, // 1800帧 = 30秒@60fps
  userDescription: "用户预估30秒视频"
})
```
...
媒体项目的原始尺寸信息（originalWidth/originalHeight）采用**多来源、渐进式精确**的设计：
...
```typescript
// 获取单独的宽度和高度
const width = mediaItem.getOriginalWidth()   // number | undefined
const height = mediaItem.getOriginalHeight() // number | undefined

// 获取尺寸对象
const size = mediaItem.getOriginalSize() // { width: number; height: number } | undefined

// 检查是否有尺寸信息
const hasSize = mediaItem.hasSize() // boolean
```

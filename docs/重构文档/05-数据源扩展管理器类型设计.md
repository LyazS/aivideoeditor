...
基于DataSourceManager基础抽象类，实现具体的管理器类型，包括用户选择文件管理器和远程文件管理器。
...
class RemoteFileManager extends DataSourceManager<RemoteFileSource> {
  ...
  protected async executeTask(task: AcquisitionTask<RemoteFileSource>): Promise<void> {
    ...
    try {
      ...
      source.setError(`下载失败: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  private async readResponseWithProgress(
    ...
    while (true) {
      ...
      if (totalLength > 0) {
        ...
        source.updateProgress(progress, `已下载 ${receivedLength}/${totalLength} 字节`)
      }
    }

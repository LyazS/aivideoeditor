...
// 1. 创建媒体项目（初始状态：pending）
const item = new UnifiedMediaItem({
  source: new RemoteSource(url),
  mediaStatus: 'pending'
})

// 2. 开始下载
await downloadManager.startDownload(item) // pending → asyncprocessing → webavdecoding
...
```typescript
// 1. 从项目配置恢复媒体项目
const item = restoreFromProject(projectConfig)

// 2. 检查本地文件存在性
if (fileExists(item.source.file)) {
  item.transitionTo('webavdecoding')
  await webavProcessor.parseFile(item) // → ready
} else {
  item.transitionTo('missing') // 文件缺失
}
```

...
3. **简化媒体项目数据结构**
   - **优化方案**：直接使用 `UnifiedMediaItemData` 现有的 `readonly id` 字段作为引用ID
   - 通过现有的 `id` 连接到页面级媒体管理器，获取完整的存储信息、元数据等
   - 避免在媒体项目中冗余存储多个字段（`storedPath`、`metadata`、`saveError`）
   - 保持数据结构简洁，完全不破坏现有的响应式数据结构
   - 实现职责分离：媒体项目专注业务逻辑，存储管理交给专门的媒体管理器

   ```typescript
   // 无需修改现有接口，直接使用已有的 readonly id
   export interface UnifiedMediaItemData {
     readonly id: string  // 既是媒体项目ID，也是媒体管理器的引用ID
     // ... 其他现有字段 ...
   }
   
   // 使用方式：通过引用ID获取完整存储信息
   const mediaRef = await globalProjectMediaManager.getMediaReference(mediaItem.id)
